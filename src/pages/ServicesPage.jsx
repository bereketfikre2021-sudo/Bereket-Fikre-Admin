...
export default function ServicesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    onSuccess: () => setLocalServices(null),
  });

  const services = localServices ?? data?.data ?? [];

  const reorderMutation = useMutation({
    mutationFn: (items) => api.put('/admin/services/reorder', { items }),
    onError: () => {
  ...
  const handleDragEnd = ({ active, over }) => {
    ...
    reorderMutation.mutate(reordered.map((s, i) => ({ id: s.id, displayOrder: i })));
  };
