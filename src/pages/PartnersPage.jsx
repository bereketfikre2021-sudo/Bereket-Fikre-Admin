...
function TestimonialModal({ t, onClose, onSave }) {
  const [form, setForm] = useState({
    ...
    featured: t?.featured || false, isActive: t?.isActive !== false,
  });
...
export default function PartnersPage() {
  ...
  const [localPartners, setLocalPartners] = useState(null);
  ...
  const partners = localPartners ?? partnersData?.data ?? [];
  const testimonials = localTestimonials ?? testimonialsData?.data ?? [];

  // ── Reorder mutations ──
  const reorderPartnersMutation = useMutation({
    mutationFn: (items) => api.put('/admin/partners/reorder', { items }),
    onError: () => { toast.error('Reorder failed'); setLocalPartners(null); },
  });

  const reorderTestimonialsMutation = useMutation({
    mutationFn: (items) => api.put('/admin/testimonials/reorder', { items }),
    onError: () => { toast.error('Reorder failed'); setLocalTestimonials(null); },
  });

  const handlePartnerDragEnd = ({ active, over }) => {
    ...
    reorderPartnersMutation.mutate(reordered.map((p, i) => ({ id: p.id, displayOrder: i })));
  };

  const handleTestimonialDragEnd = ({ active, over }) => {
    if (!over || active.id === over.id) return;
    const oldIndex = testimonials.findIndex((t) => t.id === active.id);
    const newIndex = testimonials.findIndex((t) => t.id === over.id);
    const reordered = arrayMove(testimonials, oldIndex, newIndex);
    setLocalTestimonials(reordered);
    reorderTestimonialsMutation.mutate(reordered.map((t, i) => ({ id: t.id, displayOrder: i })));
  };
    ...
    <div className="space-y-8">
      ...
      <div>
        <PageHeader
          ...
          subtitle={`${partners.length} partners — drag to reorder`}
          action={
        ...
        {loadingPartners ? <LoadingSpinner /> : (
          <DndContext sensors={sensors} collisionDetection={closestCenter} modifiers={[restrictToVerticalAxis]} onDragEnd={handlePartnerDragEnd}>
                    ...
                    {(handle) => (
                      <div className="card p-3 flex items-center gap-3">
                        ...
                        <InlineThumbnail
                          ...
                          fieldName="logo"
                          onSuccess={(updated) => qc.setQueryData(['partners'], (old) => old ? { ...old, data: old.data.map((x) => x.id === updated.id ? { ...x, logo: updated.logo } : x) } : old)}
                          size="w-10 h-10"
                          shape="rounded-lg"
                        />
      ...
      <div>
        <PageHeader
          ...
          subtitle={`${testimonials.length} testimonials — drag to reorder`}
          action={
        ...
        {loadingTestimonials ? <LoadingSpinner /> : (
          <DndContext sensors={sensors} collisionDetection={closestCenter} modifiers={[restrictToVerticalAxis]} onDragEnd={handleTestimonialDragEnd}>
                    ...
                    {(handle) => (
                      <div className="card p-3 flex items-center gap-3">
                        ...
                        <InlineThumbnail
                          ...
                          onSuccess={(updated) => qc.setQueryData(['testimonials'], (old) => old ? { ...old, data: old.data.map((x) => x.id === updated.id ? { ...x, profileImage: updated.profileImage } : x) } : old)}
                          size="w-10 h-10"
                          shape="rounded-full"
                        />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <p className="font-medium text-gray-900 dark:text-white text-sm truncate">{t.clientName}</p>
                            {t.featured && <span className="text-xs text-brand-600 dark:text-brand-400 font-medium">★ Featured</span>}
                            {!t.isActive && <span className="text-xs text-red-500">Inactive</span>}
                          </div>
                          <p className="text-xs text-gray-400 truncate">{t.company}{t.position ? ` · ${t.position}` : ''}</p>
                          <div className="flex items-center gap-0.5 mt-0.5">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <svg key={i} className={`w-2.5 h-2.5 ${i < t.rating ? 'text-yellow-400' : 'text-gray-200 dark:text-gray-700'}`} fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div className="flex gap-1 flex-shrink-0">
                          <button onClick={() => setTestimonialModal(t)} className="p-1.5 text-gray-400 hover:text-brand-600 rounded-lg hover:bg-brand-50 dark:hover:bg-brand-900/20">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                          </button>
                          <button onClick={() => setDeleteTarget({ type: 'testimonials', id: t.id })} className="p-1.5 text-gray-400 hover:text-red-500 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                          </button>
                        </div>
                      </div>
